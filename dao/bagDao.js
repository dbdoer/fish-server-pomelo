 ...
 * h_user 写档
 */
BagDao.write = function (user, cb){
    //初始化设置玩家信息保存到数据库
    var mng = user.contMng;
    var guid = user.getUid();
    var writeData = mng.bag.writeDataToDbByPropId();
...
BagDao.load = function (user, cb){
    ...
    redisCmd.hgetall(cmd, function (err, reply) {
        ...
        //console.log("-------------------101reply",reply);
        user.contMng.bag.fromDb(reply);
        cb(null, {});
    });
}
 ...
 *删除指定OBJ
 */
BagDao.delete = function (user, key, cb){
